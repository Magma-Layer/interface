...
import { MAGMA_PERMIT2_ADDRESS } from 'constants/chains'
...
// import {MAGMA_PERMIT2_ADDRESS} from
enum ApprovalState {
  PENDING,
  SYNCING,
  SYNCED,
...
export default function usePermit2Allowance(
  ...
  const { tokenAllowance, isSyncing: isApprovalSyncing } = useTokenAllowance(token, account, MAGMA_PERMIT2_ADDRESS)
  const updateTokenAllowance = useUpdateTokenAllowance(amount, MAGMA_PERMIT2_ADDRESS)
  const revokeTokenAllowance = useRevokeTokenAllowance(token, MAGMA_PERMIT2_ADDRESS)
  ...
  const isApprovalPending = useHasPendingApproval(token, MAGMA_PERMIT2_ADDRESS)
  const isRevocationPending = useHasPendingRevocation(token, MAGMA_PERMIT2_ADDRESS)
