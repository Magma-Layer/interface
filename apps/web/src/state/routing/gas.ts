...
export async function getApproveInfo(
  ...
  try {
    const allowance = await tokenContract.callStatic.allowance(account, MAGMA_PERMIT2_ADDRESS)
  ...
  try {
    const approveTx = await tokenContract.populateTransaction.approve(MAGMA_PERMIT2_ADDRESS, MaxUint256)
